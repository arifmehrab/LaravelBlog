...
<div class="row">
	<div class="col-12">
		<div class="card">
			 <div class="card-body">
			 	<a class="btn btn-primary my-3" href="{{ route('admin.tag.create') }}">
			 		<i class="fa fa-plus-circle"></i>Add Tag
			 	</a>
                <h4 class="text-right" style="font-size: 25px; font-weight: bold;">Total Tags:- {{ $tages->count() }}
                </h4>
			 		...
			 		<table id="myTable" class="table table-bordered table-striped">
                        ...
                        <tbody>
                        	...
                        	<tr>
                        		...
                        		<td>{{ $tage->slug }}</td>
                        		<td>
                        			<a title="Edit" class="btn btn-success" href="{{ route('admin.tag.edit', $tage->id) }}">
                        				<i class="fa fa-edit"></i>
                        			</a>
                      	            <button title="Delete" type="button" class="btn btn-danger" onclick="deleteTag({{ $tage->id }})">
                      	            	<i class="fa fa-trash"></i>
                      	            </button>
                      	            <form style="display: none;" id="delete_form_{{ $tage->id }}" method="post" action="{{ route('admin.tag.destroy', $tage->id) }}">
                      	            	@csrf
                      	            	@method('DELETE')
                      	            </form>
                        		</td>
